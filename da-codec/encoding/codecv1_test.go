...
func TestCodecV1BatchEncode(t *testing.T) {
	...
	encoded := hex.EncodeToString(daBatchV1.Encode())
...
func TestCodecV1BatchHash(t *testing.T) {
	...
	assert.Equal(t, common.HexToHash("0x4b6fe410f63051f6e93532087b42ece79fb7b966e2ba5845e6cd1c091f27e564"), daBatchV1.Hash())
...
func TestCodecV1NewDABatchFromBytes(t *testing.T) {
	codecv1, err := CodecFromVersion(CodecV1)
	require.NoError(t, err)
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.jsonFile == "" {
				...
				daBatch, createErr1 = codecv1.NewDABatch(batch)
			...
			decodedDABatch, createErr2 := codecv1.NewDABatchFromBytes(encodedBytes)
	...
	encodedBytes := daBatch.Encode()
	decodedDABatch, err := codecv1.NewDABatchFromBytes(encodedBytes)
	assert.NoError(t, err)
...
func TestCodecV1BlobEncodingAndHashing(t *testing.T) {
	...
	daBatch, err := codecv1.NewDABatch(batch)
	assert.NoError(t, err)
	encoded := strings.TrimRight(hex.EncodeToString(daBatch.(*daBatchV1).blob[:]), "0")
	...
	assert.Equal(t, common.HexToHash("0x01af944924715b48be6ce3c35aef7500a50e909265599bd2b3e544ac59fc7553"), daBatch.(*daBatchV1).blobVersionedHash)
	...
	assert.Equal(t, common.HexToHash("0x010c54fa675ed1b78f269827177019b0814a4ac4d269c68037e2c41cf08f9411"), daBatch.(*daBatchV1).blobVersionedHash)
	...
	assert.Equal(t, common.HexToHash("0x01ea66c4de196d36e2c3a5d7c0045100b9e46ef65be8f7a921ef20e6f2e99ebd"), daBatch.(*daBatchV1).blobVersionedHash)
	...
	assert.Equal(t, common.HexToHash("0x01a327088bb2b13151449d8313c281d0006d12e8453e863637b746898b6ad5a6"), daBatch.(*daBatchV1).blobVersionedHash)
	...
	// 15 chunks
	batch = &Batch{Chunks: []*Chunk{chunk2, chunk2, chunk2, chunk2, chunk2, chunk2, chunk2, chunk2, chunk2, chunk2, chunk2, chunk2, chunk2, chunk2, chunk2}}
	daBatch, err = codecv1.NewDABatch(batch)
	assert.NoError(t, err)
	encoded = strings.TrimRight(hex.EncodeToString(daBatch.(*daBatchV1).blob[:]), "0")
	assert.Equal(t, // metadata
	...
	assert.Equal(t, common.HexToHash("0x01521b20f341588dea5978efb00d7b077a986598a6001fc2e5859d77f3ffc284"), daBatch.(*daBatchV1).blobVersionedHash)
	...
	assert.Equal(t, common.HexToHash("0x01b63f87bdd2caa8d43500d47ee59204f61af95339483c62ff436c6beabf47bf"), daBatch.(*daBatchV1).blobVersionedHash)
}
